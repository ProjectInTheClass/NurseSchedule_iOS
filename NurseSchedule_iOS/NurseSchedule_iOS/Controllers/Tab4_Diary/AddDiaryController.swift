...
class AddDiaryController: UITableViewController {
    ...
    var new = Day(emoji: "default", date: "\(Date.init())", content: "default")
    ...
    @IBAction func clickedSaveButton(_ sender: Any) {
        ...
        if let writtenContent = diaryContent.text {
            if writtenContent.isEmpty{
                ...
                self.new.content = writtenContent
                self.new.emoji = self.seletedCondition
                DBDiary.newDiary.addDiary(userID: self.currentUser, newDiary: self.new)
    ...
    func showAlert(style : UIAlertController.Style){
        ...
        let save = UIAlertAction(title: "저장", style: .default){(action) in
            ...
            //디비에 내용 저장
            self.new.content = self.writtenContent!
            self.new.emoji = self.seletedCondition
            DBDiary.newDiary.addDiary(userID: self.currentUser, newDiary: self.new)
            self.dismiss(animated: true, completion: nil)
